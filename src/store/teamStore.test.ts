...
it('updateTransaction은 해당 거래를 교체하고 요약을 재계산한다', () => {
  ...
  addTransaction({ merchant: 'a', type: 'expense', description: '', category: 'meal', amount: 1000, date: '2026-06-01' });
  const id = useTeamStore.getState().transactions[0]!.id;
  updateTransaction(id, { merchant: 'a수정', type: 'expense', description: '', category: 'meal', amount: 3000, date: '2026-06-01' });
...
it('deleteTransaction은 거래를 제거하고 요약을 재계산한다', () => {
  ...
  addTransaction({ merchant: 'a', type: 'expense', description: '', category: 'meal', amount: 1000, date: '2026-06-01' });
  addTransaction({ merchant: 'b', type: 'income', description: '', category: 'membership', amount: 5000, date: '2026-06-02' });
