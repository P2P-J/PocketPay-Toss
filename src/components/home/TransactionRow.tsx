...
interface Props {
  tx: Transaction;
  onPress?: (tx: Transaction) => void; // 탭 → 수정
  onLongPress?: (tx: Transaction) => void; // 꾹 → 삭제
}

// HTML 레퍼런스와 동일한 행: 좌측 거터에 flush, 아이콘~글자 gap 12.
// (TDS ListRow는 자체 paddingHorizontal:24가 있어 본문 거터와 합쳐 안쪽으로 밀림 → 커스텀 행 사용)
export function TransactionRow({ tx, onPress, onLongPress }: Props) {
  const amountColor = tx.type === 'income' ? colors.income : colors.expense;
  const interactive = !!(onPress || onLongPress);
  const Container: React.ElementType = interactive ? Pressable : View;
  return (
    <Container
      style={styles.row}
      {...(interactive
        ? { onPress: () => onPress?.(tx), onLongPress: () => onLongPress?.(tx), delayLongPress: 400 }
        : {})}
    >
