...
// 컴포넌트 자체 로직(부호/색/라벨)을 검증하기 위해 ListRow를
...
jest.mock('@toss/tds-react-native', () => {
  ...
  function ListRow({ left, contents, right }: any) {
    return ReactLocal.createElement(RN.View, null, left, contents, right);
  }
  ListRow.Texts = ({ top, bottom }: any) =>
    ReactLocal.createElement(
      RN.View,
      null,
      ReactLocal.createElement(RN.Text, null, top),
      ReactLocal.createElement(RN.Text, null, bottom),
    );
  ListRow.RightTexts = ({ top }: any) =>
    ReactLocal.createElement(RN.Text, null, top);
